...
En este estado se pueden observar muchos mas elementos en el HMI, esto se debe a que estos son usados para hacer la animación de la banda transportadora en funcionamiento y clasificando de acuerdo al color, esto se hizo gracias a las variables de estado de los objetos especificamente la de "Invisible", esta hace que un objeto sea invisible cuando la variable a la cual se le asigno esta TRUE (Estado 1). Con esto se animaron los siguientes objetos:
  ...
  - La ficha despues de pasar el detector de color con el color detectado, esto se hizo asociando las variables de color a variables exclusivas para el HMI como lo seria C1Visual - C3Visual, variables que se dejan en estado de set dependiendo del boton de color presionado y se reinician al señalizar que viene una nueva pieza, para que asi la ficha que aparezca sea la del color correspondiente. Asi mismo se crearon la variables Color"colorquetienelapieza" (Ej: ColorAzul), esta se activa en el momento que se cumple la condición de que la ficha ya se le asigno un color y el actuador del sensor de proximidad tambien fue activado, se ponen en estado de NOT para que solo aparezcan cuando dichas variables sean TRUE.
